...
class SomitiSeeder extends Seeder
{
    public function run(): void
    {
        ...
        Somiti::factory(3)->create()->each(function (Somiti $somiti) use ($admin) {
            ...
            // Financial years - create two deterministic distinct years to avoid unique constraint collisions
            $years = collect();
            $base = \Carbon\Carbon::now()->startOfYear();
            for ($i = 0; $i < 2; $i++) {
                ...
                $years->push(FinancialYear::firstOrCreate(
                    ...
                    ['start_date' => $s->format('Y-m-d'), 'end_date' => $e->format('Y-m-d'), 'is_active' => $i === 0]
                ));
            }
