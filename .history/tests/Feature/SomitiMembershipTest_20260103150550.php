...
class SomitiMembershipTest extends TestCase
{
    ...
    public function test_manager_can_add_and_update_member()
    {
        ...
        $response = $this->putJson(route('somitis.users.update', ['somiti' => $somiti->id, 'user' => $user->id]), ['role' => 'auditor']);
        $response->assertStatus(200);

        $this->assertDatabaseHas('somiti_members', ['somiti_id' => $somiti->id, 'user_id' => $user->id, 'role' => 'auditor']);
    }
